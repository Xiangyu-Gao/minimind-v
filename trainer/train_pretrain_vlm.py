...
def train_epoch(epoch, loader, iters, start_step=0, wandb=None):
    ...
    for step, (X, Y, loss_mask, pixel_values) in enumerate(
        ...
        if (step % args.save_interval == 0 or step == iters - 1) and is_main_process():
            ...
            clean_state_dict = {
                k: v.half().cpu() for k, v in clean_state_dict.items()
            }  # 半精度保存并移到CPU
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--save_weight", default="pretrain_vlm", type=str, help="保存权重的前缀名"
    )
    ...
    parser.add_argument(
        "--max_seq_len", default=640, type=int, help="训练的最大截断长度"
    )
    parser.add_argument(
        "--use_moe",
        default=0,
    ...
    parser.add_argument(
        "--images_path",
        type=str,
        default="../dataset/pretrain_images",
        help="训练图像路径",
    )
    parser.add_argument(
        "--from_weight",
    ...
    parser.add_argument(
        ...
        type=int,
        choices=[0, 1],
        help="是否冻结LLM参数（0=否，1=是，仅训练vision_proj）",
    )
    parser.add_argument("--use_wandb", action="store_true", help="是否使用wandb")
    parser.add_argument(
        "--wandb_project", type=str, default="MiniMind-V-Pretrain", help="wandb项目名"
    )
